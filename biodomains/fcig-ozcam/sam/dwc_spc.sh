...
#  echo distinct \(\(select CatDiscipline from ecatalogue\)\) | texql -R | tr -d \'\(\) >> "$DWCDM/disciplines-list"
  echo distinct \(select CatCollectionName_tab[CatCollectionName] from ecatalogue\) | texql -R | tr -d \'\(\)[] >> "$DWCDM/disciplines-list"
...
texlist -l | cut --characters=-18 > "$DWCDM/$EXDIR/tab-list"
